...
for e in selected_stages:
    ...
    for s in scenarios:
        ...
        # Transport varies with capacity. XD / SC are fixed: variation = 0%.
        row[f"Var. {s}"]=0 if e in ["XD","Service Center"] else (stage_cost(e,s)/NORMAL[e]-1)*100
    rows.append(row)
...
with left:
    ...
    colors=[BLUE]+[SCOL[s] for s in scenarios]
    texts=[ars(round(v),0) for v in totals]
...
with right:
    ...
    for c in d.columns:
        if c=="Etapa":
            continue
        if c.startswith("Var."):
            d[c]=d[c].map(lambda x: "0%" if abs(x) < 0.5 else ("+" if x>0 else "")+f"{x:.0f}%")
        else:
            # XD y Service Center se muestran con 2 decimales; el resto, sin decimales.
            d[c] = [
                ars(v,2) if etapa in ["XD","Service Center"] else ars(round(v),0)
                for etapa,v in zip(d["Etapa"], d[c])
            ]

    # Rename variation headers for faster executive reading.
    d=d.rename(columns={c:c.replace("Var. ","Δ vs normal ") for c in d.columns if c.startswith("Var. ")})

    st.dataframe(
        d, hide_index=True, use_container_width=True,
        height=min(315,45+35*len(d)),
        column_config={
            "Etapa": st.column_config.TextColumn("Etapa", width="medium"),
            "Normal": st.column_config.TextColumn("Normal", width="small"),
            **{s: st.column_config.TextColumn(s, width="small") for s in scenarios},
            **{f"Δ vs normal {s}": st.column_config.TextColumn("Δ "+s, width="small") for s in scenarios},
        }
    )
...
if stage=="Todas":
    ...
    # Exact fixed-center values: pies communicate composition; this line preserves exact economics.
    exact_cols=st.columns(len(donut_scenarios))
    for ec,sc in zip(exact_cols,donut_scenarios):
        with ec:
            if sc=="Normal":
                xd_v=NORMAL["XD"]; sc_v=NORMAL["Service Center"]
            else:
                xd_v=XD_AFTER; sc_v=SC_AFTER
            st.markdown(
                f'<div style="text-align:center;font-size:11px;color:#111;line-height:1.55;margin-top:4px;">'
                f'<b>XD</b> {ars(xd_v,2)} &nbsp;·&nbsp; <b>SC</b> {ars(sc_v,2)}</div>',
                unsafe_allow_html=True
            )
...
if stage=="Todas" or stage in ["XD","Service Center"]:
    ...
    fixed=pd.DataFrame({
        "Centro":centers,
        "Costo promedio normal":[ars(round(NORMAL[e]),0) for e in centers],
        "Promedio con voluminosos":[ars(round(XD_AFTER),0) if e=="XD" else ars(round(SC_AFTER),0) for e in centers],
