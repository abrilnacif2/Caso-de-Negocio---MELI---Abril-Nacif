...
if cap_stages:
    ...
    with right:
        ...
        st.markdown('<div class="callout"><b>Lectura:</b> las capacidades equivalentes se muestran en paquetes completos; cualquier decimal se redondea hacia abajo.</div>',unsafe_allow_html=True)
else:
...
<div class="section-head">
 ...
 <div class="deck">Costo promedio por paquete en transporte y centros fijos.</div>
</div>
""",unsafe_allow_html=True)
...
with left:
    # Total cost comparison: one message, no clutter.
    labels=["Normal"]+scenarios
    totals=[sum(NORMAL.values())]+[avg_total(s) for s in scenarios]
    colors=[BLUE]+[SCOL[s] for s in scenarios]
    fig=go.Figure(go.Bar(
        x=labels,y=totals,width=.46,marker_color=colors,
        text=[ars(v) for v in totals],textposition="outside",
        textfont=dict(color=INK,size=13),cliponaxis=False,
        hovertemplate="<b>%{x}</b><br>Costo total: $%{y:,.0f}/paq.<extra></extra>"
    ))
    executive_layout(fig,300,False,dict(l=10,r=10,t=25,b=20))
...
with right:
    ...
    st.dataframe(d,hide_index=True,use_container_width=True,height=min(310,45+35*len(d)))

# Cost mix: only when transport stages are in view.
transport_stages=[e for e in selected_stages if e in TRANS]
if transport_stages:
    mix_scenarios=scenarios
    fig=go.Figure()
    stage_colors={"Colecta":"#5AC8FA","Media milla":"#FF9F0A","Última milla":"#24364B"}
    for e in transport_stages:
        vals=[]
        for s in mix_scenarios:
            tc=transport_cost(s)
            vals.append((NORMAL[e]*FACT[s]/tc)*100)
        fig.add_trace(go.Bar(
            name=e,x=mix_scenarios,y=vals,marker_color=stage_colors[e],
            hovertemplate=f"<b>{e}</b><br>%{{y:.1f}}% del costo de transporte<extra></extra>"
        ))
    executive_layout(fig,255,True,dict(l=10,r=10,t=35,b=15))
    fig.update_layout(barmode="stack")
    fig.update_yaxes(title="Participación en costo de transporte",ticksuffix="%",range=[0,100],dtick=25)
    fig.update_xaxes(title="",showgrid=False)
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
...
with right:
    ...
    with c2:
        st.markdown('<div class="metric-card"><div class="metric-label">Revenue relativo</div><div class="metric-value">6,33x</div><div class="metric-note">+$533% vs. normal</div></div>',unsafe_allow_html=True)
...
# Final management takeaway: concise, not a duplicate KPI section.
st.markdown(f"""
<div class="callout" style="margin-top:18px;border-left:5px solid #FFE600;padding:16px 18px;">
<b>Lectura ejecutiva:</b> el proyecto mantiene resultado positivo en 5x y 5,5x, y pasa a negativo en 6x.
El punto de equilibrio es <b>{dec(BE,2)}x</b>. En el escenario 5,5x, el resultado incremental es
<b>{ars(metrics("5,5x")[1])} por paquete</b>.
</div>
<div class="callout" style="margin-top:10px;">
<b>Nota de costos fijos:</b> XD y Service Center mantienen su costo promedio de red.
Su costo incremental es $0 bajo el supuesto de capacidad ociosa.
</div>
""",unsafe_allow_html=True)
