...
# ---------------- SCENARIO SCORECARDS ----------------
st.markdown('<div class="section-title">Escenarios</div><div class="section-sub">Los tres escenarios permanecen visibles; el selector solo destaca uno.</div>',unsafe_allow_html=True)
cols=st.columns(3)
...
# Revenue: direct comparison from case tariff
revcol, costcol = st.columns([0.82, 1.7])

with revcol:
    st.markdown('<div class="section-sub">Revenue por paquete</div>',unsafe_allow_html=True)
    revdf=pd.DataFrame({
        "Tipo":["Normal","Voluminoso"],
        "Revenue":[3000,19000]
    })
    fig_rev=go.Figure()
    fig_rev.add_trace(go.Bar(
        x=revdf["Tipo"], y=revdf["Revenue"],
        marker_color=[BLUE,YELLOW], width=0.38,
        text=[ars(v) for v in revdf["Revenue"]],
        textposition="outside", cliponaxis=False,
        hovertemplate="<b>%{x}</b><br>Revenue: $%{y:,.0f}<extra></extra>"
    ))
    fig_rev.update_layout(
        height=330, margin=dict(l=10,r=10,t=20,b=15), showlegend=False,
        xaxis=dict(title="",tickfont=dict(size=13,color=INK),showgrid=False),
        yaxis=dict(title="$ / paquete",gridcolor="#EEF0F3",range=[0,22000]),
...
with costcol:
    st.markdown(f'<div class="section-sub">Costo por etapa · Normal vs. voluminoso ({focus})</div>',unsafe_allow_html=True)
    # For transport, selected bulky scenario cost. For fixed centers, average network cost after adding 50k.
    xd_after=741000000/10050000
    sc_after=741500000/10050000
    compare_rows=[]
    for e in ["Colecta","XD","Media milla","Service Center","Última milla"]:
        if e in TRANS:
            bulky=NORMAL[e]*FACT[focus]
        elif e=="XD":
            bulky=xd_after
        else:
            bulky=sc_after
        compare_rows.append({"Etapa":e,"Normal":NORMAL[e],"Voluminoso":bulky})
    comp=pd.DataFrame(compare_rows)

    fig_cmp=go.Figure()
    fig_cmp.add_trace(go.Bar(
        name="Normal",x=comp["Etapa"],y=comp["Normal"],marker_color=BLUE,width=0.18,
        hovertemplate="<b>Normal</b><br>%{x}: $%{y:,.0f}<extra></extra>"
    ))
    fig_cmp.add_trace(go.Bar(
        name=f"Voluminoso · {focus}",x=comp["Etapa"],y=comp["Voluminoso"],marker_color=YELLOW,width=0.18,
        hovertemplate=f"<b>Voluminoso · {focus}</b><br>%{{x}}: $%{{y:,.0f}}<extra></extra>"
    ))
    fig_cmp.update_layout(
        barmode="group",height=330,margin=dict(l=10,r=10,t=20,b=15),
        legend=dict(orientation="h",y=1.12,x=.5,xanchor="center",font=dict(color=INK)),
        xaxis=dict(title="",tickfont=dict(size=12,color=INK)),
        yaxis=dict(title="$ / paquete",gridcolor="#EEF0F3"),
        plot_bgcolor="white",paper_bgcolor="white"
    )
    st.plotly_chart(fig_cmp,use_container_width=True,config={"displayModeBar":False})

# Compact comparison table, explicit about fixed-cost concept
normal_total=sum(NORMAL.values())
bulky_transport=sum(NORMAL[e]*FACT[focus] for e in TRANS)
bulky_avg_network=bulky_transport+xd_after+sc_after
comparison=pd.DataFrame({
    "Indicador":["Revenue / paquete","Costo transporte / paquete","XD promedio / paquete","SC promedio / paquete"],
    "Normal":[ars(3000),ars(sum(NORMAL[e] for e in TRANS)),"$"+dec(NORMAL["XD"],2),"$"+dec(NORMAL["Service Center"],2)],
    f"Voluminoso · {focus}":[ars(19000),ars(bulky_transport),"$"+dec(xd_after,2),"$"+dec(sc_after,2)],
    "Relación / variación":["6,33x",f"{FACT[focus]:.1f}x".replace(".",","),"≈ −0,5%","≈ −0,5%"]
})
...
with left:
    ...
    fig.add_trace(go.Bar(x=sc,y=monthly_vals,
        marker_color=[SCOL[s] for s in sc], width=0.42,
...
# ---------------- COSTS ----------------
st.markdown('<div class="section-title">Costos de transporte · sensibilidad por escenario</div>',unsafe_allow_html=True)
transport_rows=[]
...
with left:
    ...
    for s,color in [("Normal",BLUE),("5x",GREEN),("5,5x",ORANGE),("6x",RED)]:
        fig.add_trace(go.Bar(name=s,x=tdf["Etapa"],y=tdf[s],marker_color=color, width=0.14,
...
fig=go.Figure()
for s,color in [("Normal",BLUE),("5x",GREEN),("5,5x",ORANGE),("6x",RED)]:
    fig.add_trace(go.Bar(name=s,x=cdf["Etapa"],y=cdf[s],marker_color=color, width=0.14,
