...
transport_rows=[]
transport_stages=TRANS if stage=="Todas" else ([stage] if stage in TRANS else [])
...
with left:
    if not tdf.empty:
        fig=go.Figure()
        visible_series=["Normal"] + (list(FACT.keys()) if focus=="Todos" else [focus])
        cmap={"Normal":BLUE,"5x":GREEN,"5,5x":ORANGE,"6x":RED}
        for s in visible_series:
            fig.add_trace(go.Bar(name=s,x=tdf["Etapa"],y=tdf[s],marker_color=cmap[s],width=0.14))
        ...
        st.markdown('<div class="card"><div class="k-label">COSTO DE TRANSPORTE</div><div class="k-val">—</div><div class="k-sub">La etapa seleccionada es un centro fijo.</div></div>',unsafe_allow_html=True)
with right:
    xd_after=741000000/10050000; sc_after=741500000/10050000
    centers=["XD","Service Center"] if stage=="Todas" else ([stage] if stage in ["XD","Service Center"] else [])
    fixed=pd.DataFrame({
        "Centro":centers,
        "Normal":[NORMAL[e] for e in centers],
        "Con voluminosos":[xd_after if e=="XD" else sc_after for e in centers],
        "Incremental":[0 for _ in centers]
    })
    if not fixed.empty:
        fixed["Normal"]=fixed["Normal"].map(lambda x:"$"+dec(x,2))
        fixed["Con voluminosos"]=fixed["Con voluminosos"].map(lambda x:"$"+dec(x,2))
        fixed["Incremental"]=["$0" for _ in range(len(fixed))]
        st.dataframe(fixed,hide_index=True,use_container_width=True,height=145)
    else:
        st.markdown('<div class="card"><div class="k-label">CENTROS FIJOS</div><div class="k-val">—</div><div class="k-sub">La etapa seleccionada es transporte.</div></div>',unsafe_allow_html=True)
    st.markdown("""
...
fig=go.Figure()
cap_series=["Normal"] + (list(FACT.keys()) if focus=="Todos" else [focus])
cmap={"Normal":BLUE,"5x":GREEN,"5,5x":ORANGE,"6x":RED}
for s in cap_series:
    fig.add_trace(go.Bar(name=s,x=cdf["Etapa"],y=cdf[s],marker_color=cmap[s], width=0.14,
