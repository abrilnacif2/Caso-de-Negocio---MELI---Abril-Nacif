...
with gc:
    # Use dot/line profile rather than dense grouped bars
    fig=go.Figure()
    fig.add_trace(go.Scatter(
        x=costdf["Etapa"],y=costdf["Normal"],mode="lines+markers",
        name="Normal",line=dict(color=BLUE,width=3),marker=dict(size=10)
    ))
    for s in selected_scenarios:
        fig.add_trace(go.Scatter(
            x=costdf["Etapa"],y=costdf[s],mode="lines+markers",
            name=f"Voluminoso {s}",line=dict(color=SCOL[s],width=3),marker=dict(size=10)
        ))
    fig.update_layout(height=345,margin=dict(l=10,r=10,t=25,b=15),
        legend=dict(orientation="h",y=1.15,x=.5,xanchor="center",font=dict(color=INK)),
        xaxis=dict(title="",tickfont=dict(color=INK)),
        yaxis=dict(title="$ / paquete",gridcolor="#EEF0F3",tickfont=dict(color=INK)),
        plot_bgcolor="white",paper_bgcolor="white")
    st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
with tc:
    costtab=costdf.copy()
...
if cap_stages:
    ...
    cg,ct=st.columns([1.25,1])
    with cg:
        fig=go.Figure()
        fig.add_trace(go.Scatter(x=cdf["Etapa"],y=cdf["Normal"],mode="lines+markers",
            name="Normal",line=dict(color=BLUE,width=3),marker=dict(size=10)))
        for s in selected_scenarios:
            fig.add_trace(go.Scatter(x=cdf["Etapa"],y=cdf[s],mode="lines+markers",
                name=s,line=dict(color=SCOL[s],width=3),marker=dict(size=10)))
        fig.update_layout(height=330,margin=dict(l=10,r=10,t=25,b=15),
            legend=dict(orientation="h",y=1.15,x=.5,xanchor="center",font=dict(color=INK)),
            xaxis=dict(title="",tickfont=dict(color=INK)),
            yaxis=dict(title="Equivalentes / ruta",gridcolor="#EEF0F3",tickfont=dict(color=INK)),
            plot_bgcolor="white",paper_bgcolor="white")
        st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False})
    with ct:
        captab=cdf.copy()
        for c in captab.columns[1:]:
            captab[c]=captab[c].map(lambda x:dec(x))
        st.dataframe(captab,hide_index=True,use_container_width=True,height=45+35*len(captab))
else:
    st.markdown('<div class="card"><div class="k-label">CAPACIDAD POR RUTA</div><div class="k-val">—</div><div class="k-sub">XD y Service Center son centros fijos; la capacidad por ruta aplica a transporte.</div></div>',unsafe_allow_html=True)
